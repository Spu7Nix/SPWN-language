...
macro_rules! error_maker {
    (
        ...
        impl$(<$lt>)? $enum$(<$lt>)? {
            pub fn to_report(&self $(, $extra_arg: $extra_type)* ) -> $crate::util::error::ErrorReport {
                use colored::Colorize;
                use $crate::util::error::RainbowColorGenerator;

                let mut info_colors = RainbowColorGenerator::new(166.0, 0.5, 0.95, 35.0);

                match self {
                    $(
                        $enum::$err_name { $($field,)* $($call_stack)? } => $crate::util::error::ErrorReport  {
