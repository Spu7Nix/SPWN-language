...
//         }
//     ) => {
//         #[derive(Debug, Clone, delve::EnumFromStr, delve::EnumVariantNames, delve::EnumTuples, delve::EnumModify)]
...
//                 let mut attr = match <$enum as std::str::FromStr>::from_str(&attr_name) {
//                     Ok(a) => a,
//                     Err(..) => {
//                         return Err(SyntaxError::UnknownAttribute {
//                             area: parser.make_area(attr_name_span),
//                             attribute: attr_name,
//                             valid: $enum::VARIANT_NAMES.iter().map(|v| hyperlink("https://spu7nix.net/spwn/#/attributes?id=attributes", Some(v))).collect()
//                         })
//                     }
//                 };
...
//                         let tuple_arg_amount = <$enum as delve::TupleCount>::tuple_count(&attr).unwrap();

//                         parser.expect_tok(Token::LParen)?;

//                         parse_string!(parser);

//                         <$enum as delve::ModifyField<usize, $typ1>>::set_field(&mut attr, 0, parser.slice().into());

//                         for _ in 1..tuple_arg_amount {
//                             parser.expect_tok(Token::Comma).map_err(|_| {
//                                 SyntaxError::InvalidAttributeArgCount {
//                                     attribute: stringify!([< $variant:snake >]).into(),
