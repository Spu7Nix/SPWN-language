...
mod ast; //abstract syntax tree module
...
fn main() {
    ...
    let parse_tree = SPWNParser::parse(Rule::spwn, &unparsed).expect("unsuccessful parse").next().unwrap(); // get and unwrap the `spwn` rule; never fails
    ...
    for statement in statements {
        ...
        stmts.push(match statement.as_rule() {
            ...
            Rule::event => ast::Statement::Event(ast::Event {
                symbol:   statement.clone().into_inner().next().unwrap().as_span().as_str().to_string(),
                cmp_stmt: ast::CompoundStatement {statements: parse_statements(&mut statement.into_inner().nth(1).unwrap().into_inner())}
            }),
            Rule::call => {
                ...
                ast::Statement::Call(ast::Call {value: value, symbols: symbols})
            },
...
fn parse_value(pair: Pair<Rule>) -> ast::Value {
    match pair.as_rule() {
        ...
        Rule::cmp_stmt => ast::Value::CmpStmt(ast::CompoundStatement {statements: parse_statements(&mut pair.into_inner())}),
